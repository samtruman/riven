        ...
        with db.Session() as session:
            ...
            if event.content_item:
                ...
                if indexed_item is None:
                    logger.debug(f"Unable to index {event.content_item.log_string if event.content_item.log_string is not None else (event.content_item.tmdb_id or event.content_item.imdb_id)}")
                    return None
