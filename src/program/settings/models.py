...
class RTNSettingsModel(SettingsModel, Observable):
    """RTN settings plus the one local policy RTN cannot express.

    RTN owns filtering, rank and final resolution/rank ordering.  Its parsed
    language list deliberately does not identify whether a language token is
    audio or subtitle-only, so this policy is intentionally limited to that
    ambiguity.
    """

    require_explicit_italian_tag: bool = Field(
        default=True,
        title="Require explicit Italian release tag",
        description="When Italian is required, reject a release that does not explicitly declare ITA, Italian or Italiano.",
    )
    prefer_explicit_italian_audio: bool = Field(
        default=True,
        title="Prefer explicit Italian audio",
        description="When Italian is required, use subtitle-only Italian releases only if no candidate explicitly declares Italian audio. Disable to let RTN rank all accepted releases normally.",
    )
