...
class EventManager:
    ...
    def add_event(self, event: Event) -> bool:
        ...
        if item_id:
            ...
            external_id = event.content_item.tmdb_id or event.content_item.imdb_id
            if any(event.content_item and (event.content_item.tmdb_id or event.content_item.imdb_id) == external_id for event in self._queued_events):
                logger.debug(f"Content item {external_id} is already in queue, skipping.")
                return False
            if any(event.content_item and (event.content_item.tmdb_id or event.content_item.imdb_id) == external_id for event in self._running_events):
                logger.debug(f"Content item {external_id} is already running, skipping.")
                return False
    ...
    def add_item(self, item, service: str = "Manual") -> bool:
        ...
        ) if item.tmdb_id else db_functions.get_item_by_external_id(imdb_id=item.imdb_id)
        if not existing:
            if self.add_event(Event(service, content_item=item)):
                logger.debug(f"Added requested item {item.tmdb_id or item.imdb_id} to the queue.")
