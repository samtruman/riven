...
def _magnet_entries(payload: dict) -> list[dict]:
    """Return magnet objects from either documented or legacy API shapes.

    The current v4.1 documentation describes ``magnets`` as an array.  Some
    account responses still return a mapping keyed by magnet ID, so normalize
    both forms before selecting the requested magnet.
    """
    magnets = payload.get("magnets", [])
    if isinstance(magnets, list):
        return [entry for entry in magnets if isinstance(entry, dict)]
    if isinstance(magnets, dict):
        # Some v4.1 responses for a single ``id`` unwrap the magnet object
        # directly under ``magnets`` instead of returning a one-element list.
        if "id" in magnets or "filename" in magnets:
            return [magnets]
        entries = []
        for magnet_id, entry in magnets.items():
            if isinstance(entry, dict):
                entries.append({"id": magnet_id, **entry})
        return entries
    return []
...
class AllDebridAPI:
    ...
    STATUS_BASE_URL = "https://api.alldebrid.com/v4.1"
    AGENT = "Riven"
    ...
    def __init__(self, api_key: str, proxy_url: Optional[str] = None):
        ...
        # AllDebrid moved only magnet/status to v4.1.  Keep the established
        # v4 handler for upload/files/delete, whose endpoints remain on v4.
        self.status_request_handler = AllDebridRequestHandler(
            self.session, self.STATUS_BASE_URL, base_params
        )
...
class AllDebridDownloader(DownloaderBase):
    ...
    def get_torrent_info(self, torrent_id: str) -> TorrentInfo:
        ...
        try:
            response = self.api.status_request_handler.execute(
                HttpMethod.GET, "magnet/status", params={"id": torrent_id}
            )
            magnets = _magnet_entries(response)
            # AllDebrid returns an array even when ``id`` requests a single
            # magnet.  Treating it as a mapping made every valid response
            # fail the filename check and caused cached candidates to be
            # blacklisted.
            info = next(
                (magnet for magnet in magnets if str(magnet.get("id")) == str(torrent_id)),
                {},
            )
            if "filename" not in info:
                raise AllDebridError("Invalid torrent info response")
            size = info["size"]
            downloaded = info.get("downloaded", size if info["status"] == "Ready" else 0)
            return TorrentInfo(
                id=info["id"],
                name=info["filename"],
                status=info["status"],
    ...
    def get_files_and_links(self, torrent_id: str) -> List[DebridFile]:
        ...
        try:
            ...
            magnet_info = next(
                (info for info in _magnet_entries(response) if str(info.get("id")) == str(torrent_id)),
                {},
            )
