...
_ITALIAN_TOKEN = re.compile(r"(?i)(?:^|[.\- _\[\](){}])(ita|italian|italiano)(?=$|[.\- _\[\](){}])")
_SUBTITLE_MARKERS = {"sub", "subs", "subtitle", "subtitles"}
def _italian_release_kind(raw_title: str) -> str:
    """Classify explicit Italian release tokens without reading media bytes.

    RTN currently reports subtitle languages together with audio languages.
    Audio wins over subtitle-only releases, which remain a valid fallback for
    media that was never dubbed in Italian.
    """
    saw_subtitles = False
    for match in _ITALIAN_TOKEN.finditer(raw_title):
        prefix = raw_title[:match.start(1)].rstrip(".-_ [](){}")
        # The immediately preceding release token is enough to identify the
        # common `Sub.ITA` / `Subs-Italian` forms.
        previous_token = re.split(r"[.\- _\[\](){}]+", prefix)[-1].lower() if prefix else ""
        if previous_token not in _SUBTITLE_MARKERS:
            return "audio"
        saw_subtitles = True
    return "subtitles" if saw_subtitles else "none"


def _apply_italian_audio_policy(
    torrents: Set[Torrent], italian_kinds: Dict[str, str]
) -> Set[Torrent]:
    """Keep subtitle-only Italian releases only as a documented fallback.

    This does not score or sort torrents.  RTN remains responsible for all
    normal eligibility checks and for resolution/rank ordering.  The policy is
    applied only when Italian is a required language and can be disabled from
    the existing Ranking settings.
    """
    preference = settings_manager.settings.ranking
    if (
        "it" not in set(preference.languages.required or [])
        or not preference.prefer_explicit_italian_audio
    ):
        return torrents
    audio_candidates = {
        torrent
        for torrent in torrents
        if italian_kinds.get(torrent.infohash) == "audio"
    }
    return audio_candidates or torrents
...
def _parse_results(item: MediaItem, results: Dict[str, str], log_msg: bool = True) -> Dict[str, Stream]:
    ...
    for infohash, raw_title in results.items():
        ...
        try:
            ...
            required_languages = set(ranking_settings.languages.required or [])
            italian_kind = _italian_release_kind(raw_title) if "it" in required_languages else "audio"
            if (
                "it" in required_languages
                and settings_manager.settings.ranking.require_explicit_italian_tag
                and italian_kind == "none"
            ):
                if scraping_settings.parse_debug:
                    logger.debug(f"Skipping torrent without explicit Italian tag for {item.log_string}: {raw_title}")
                continue
            ...
            italian_kinds[torrent.infohash] = italian_kind
    ...
    if torrents:
        ...
        torrents = _apply_italian_audio_policy(torrents, italian_kinds)
